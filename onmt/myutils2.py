...
class MolData(data.Dataset):
    ...
    def __getitem__(self, index):
        g = str2molgraph(self.data[index], self.lengths[index])
        h, e = extract_graph_feature(g)
        return (g, h, e), torch.tensor([1])
...
def collate_g(batch):
    ...
    for i in range(len(batch)):
        ...
        num_nodes = len(batch[i][1])

        # Adjacency matrix
        g[i, 0:num_nodes, 0:num_nodes] = batch[i][0]
       
        # Node features
        h[i, 0:num_nodes, :] = batch[i][1]

        # Edges
        for edge in batch[i][2].keys():
            e[i, edge[0], edge[1], :] = batch[i][2][edge]
            e[i, edge[1], edge[0], :] = batch[i][2][edge]
...
invalid_words = set(['<unk>', '<RX_9>', '<RX_5>', '<RX_2>', '<blank>', '<RX_10>', '<RX_8>', '</s>', '<RX_6>', '<s>', '<RX_3>', '<RX_4>', '<RX_1>', '<RX_7>'])
...
def str2molgraph(rawstr, length):  # rawstr :tuple() e.g. ('<RX_6>', 'N', 'c', '1', 'n', 'c', '2', '[', 'n', 'H', ']', 'c', '(', 'C', 'C', 'C', 'c', '3', 'c', 's', 'c', '(', 'C', '(', '=', 'O', ')', 'O', ')', 'c', '3', ')', 'c', 'c', '2', 'c', '(', '=', 'O', ')', '[', 'n', 'H', ']', '1')    
    ...
    for i in range(0, len(feats)):
        if feats[i].GetFamily() == 'Donor':
            node_list = feats[i].GetAtomIds()
            for i in node_list:
                g.node[atom_true_index[i]]['donor'] = 1
        elif feats[i].GetFamily() == 'Acceptor':
            node_list = feats[i].GetAtomIds()
            for i in node_list:
                g.node[atom_true_index[i]]['acceptor'] = 1
    ...
    for i in range(0, m.GetNumAtoms()):
        for j in range(0, m.GetNumAtoms()):
            ...
            if e_ij is not None:
                ...
                # Unbonded
                g.add_edge(atom_true_index[i], atom_true_index[j], b_type=None,)

    return g


rawstr = ('<RX_6>', 'N', 'c', '1', 'n', 'c', '2', '[', 'n', 'H', ']', 'c', '(', 'C', 'C', 'C', 'c', '3', 'c', 's', 'c', '(', 'C', '(', '=', 'O', ')', 'O', ')', 'c', '3', ')', 'c', 'c', '2', 'c', '(', '=', 'O', ')', '[', 'n', 'H', ']', '1')    
# rawstr2 = '<RX_1> C C C C C C C N C ( = O ) N ( C ) c 1 c c c c ( - c 2 c c c ( C C C ( = O ) O C ) c c 2 O C C C Cl ) c 1'.split(' ')
import dgl
from dgl.nn.pytorch.conv import EGATConv
from GATGATE import *


g1 = str2molgraph(rawstr, len(rawstr))
h1, e1 = extract_graph_feature(g1)
print(e1)
g = dgl.DGLGraph(g1)
print(g.nodes())
print(g.edges())
for key in e1:
    a,b = key[0],key[1]
    g.edges[a,b].data['w'] = torch.tensor(e1[key]).unsqueeze(0).float()
    g.edges[b,a].data['w'] = torch.tensor(e1[key]).unsqueeze(0).float()

g.ndata['init_h'] = torch.tensor(h1).float()
model = GATGATE(21,256,4,2)
#model = EGATConv(21,21,2)
out = model(g, g.ndata['init_h'])
print(out.size())
# g1 = nx.to_numpy_matrix(g1)
# g2 = str2molgraph(rawstr2, len(rawstr2))
# h2, e2 = extract_graph_feature(g2)
# g2 = nx.to_numpy_matrix(g2)
# batch = [(g1,h1,e1),(g2,h2,e2)]
# g,h,e = collate_g(batch)

# model = MPNN.MPNN([21, 4], 73,73,3,1)
# out = model(g,h,e)
# print(out)
# print(out.size())
